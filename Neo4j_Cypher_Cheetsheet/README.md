## Cypher Cheetsheet基础语法
  ...
  \- [6 merge关键字](#6-merge关键字)

  \- [7 更新节点](#7-更新节点)

​    \- [7.1 更新属性值](#71-更新属性值)

​    \- [7.2 新增属性和属性值](#72-新增属性和属性值)

​    \- [7.3 删除属性值](#73-删除属性值)

​    \- [7.4 更新节点类型（允许有多个标签）](#74-更新节点类型允许有多个标签)

  \- [8 匹配](#8-匹配)

​    \- [8.1 限制节点类型和属性匹配](#81-限制节点类型和属性匹配)

​    \- [8.2 可选匹配（对于缺失部分使用Null代替）](#82-可选匹配对于缺失部分使用null代替)

​    \- [8.3 字符串开头匹配](#83-字符串开头匹配)

​    \- [8.4 字符串结尾匹配](#84-字符串结尾匹配)

​    \- [8.5 字符串包含匹配](#85-字符串包含匹配)

​    \- [8.6 字符串排除匹配](#86-字符串排除匹配)

​    \- [8.7 正则匹配 =~（模糊匹配）](#87-正则匹配-模糊匹配)

​    \- [8.8 正则匹配 =~（不区分大小写）](#88-正则匹配-不区分大小写)

​    \- [8.9 属性值包含（IN）](#89-属性值包含in)

​    \- [8.10 "或"匹配（|）](#810-或匹配)

​    \- [8.11 任意节点和指定范围深度关系](#811-任意节点和指定范围深度关系)

​    \- [8.12 任意节点和指任意深度关系](#812-任意节点和指任意深度关系)

​    \- [8.13 去重返回](#813-去重返回)

​    \- [8.14 排序返回（desc降序；asc升序）](#814-排序返回desc降序asc升序)

​    \- [8.15 重命名返回](#815-重命名返回)

​    \- [8.16 多重条件限制（with），即返回认识10人以上的张%](#816-多重条件限制with即返回认识10人以上的张)

​    \- [8.17 并集去重（union）](#817-并集去重union)

​    \- [8.18 并集不去重（union all）](#818-并集不去重union-all)

​    \- [8.19 查看节点属性/ID](#819-查看节点属性id)

​    \- [8.20 匹配分页返回](#820-匹配分页返回)

  \- [9 读取文件](#9-读取文件)

​    \- [9.1 读取网络资源csv文件](#91-读取网络资源csv文件)

​    \- [9.2 分批读取网络资源](#92-分批读取网络资源)

​    \- [9.3 读取本地文件](#93-读取本地文件)

​    \- [9.4 注意事项](#94-注意事项)

  \- [10 foreach关键字](#10-foreach关键字)
